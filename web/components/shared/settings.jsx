...
import SetExpenditureWeights from "@components/shared/set_expenditure_weights";
import formatNumber from "@helpers/format_number";

export function SettingsCard() {
  const [showIncomeUpdateForm, setShowIncomeUpdateForm] = useState(false);
  const [showExpenditureWeightsForm, setShowExpenditureWeightsForm] =
    useState(false);
    ...
    <div className="z-50 py-5 px-5 absolute top-14 w-[250px] rounded-lg border-[0.1px] border-gray-200 bg-gray-50 shadow-2xl shadow-slate-300">
      ...
      <ul className="pt-2">
        <SettingsTextButton
          ...
          value={
            data
              ? `${formatNumber(data?.income?.amount) || ""} ${
                  data?.income?.currency || ""
                }`
              : null
          }
        />
        {showIncomeUpdateForm && <UpdateIncomeForm incomeData={data?.income} />}
        <SettingsTextButton
        ...
        />
        {/* https://www.buymeacoffee.com/cedricmurairi */}
        <SettingsTextButton
          action={showSomething}
        ...
        <SettingsTextButton
          action={() =>
            setShowExpenditureWeightsForm(!showExpenditureWeightsForm)
          }
          displayText="Expenditures Weights"
        />
        {showExpenditureWeightsForm && (
          <SetExpenditureWeights weights={data?.weights?.weights || {}} />
        )}
      </ul>
